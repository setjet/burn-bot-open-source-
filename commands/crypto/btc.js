...
module.exports = {
  ...
  async execute(message, args, { prefix, client, dbHelpers }) {
    ...
    // premium gate: the bouncer that made me refactor everything into premiumCheck.js 😭
    ...
    if (args.length === 0) {
          ...
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription([
                ...
                '',
                `-# Use \`${prefix}btc set\` to set your wallet address.`
              ].join('\n'))
          ],
      ...
      // blockchain apis: "here is a number" also "here is an error shaped like a number" 😭
      let balance = null;
      ...
      const embed = new EmbedBuilder()
        .setColor('#838996')
        .setDescription([
          `<:bitcoin:1464571530779431037> **<@${message.author.id}>'s Bitcoin Wallet**`,
          '',
          ...statusLines
        ].join('\n'))
        .addFields([
    ...
    async function proceedWithVerification(message, userId, currency, prefix, interaction = null) {
      ...
      // nonce + http callback + frontend — the trinity that refused to line up for days 😭
      ...
      try {
        ...
        // dms closed? classic. users blame the bot; i blame default privacy settings 😭
    ...
    // you typed `btc asdf` and expected wisdom
    return message.reply({
      embeds: [
        new EmbedBuilder()
          .setColor('#838996')
