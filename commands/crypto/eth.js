...
module.exports = {
  ...
  async execute(message, args, { prefix, client, dbHelpers }) {
    ...
    // eth: where wei formatting taught me what BigInt anxiety feels like 😭
    ...
    if (args.length === 0) {
      ...
      // etherscan said "rate limit" so often it felt personal
      let balance = null;
    ...
    async function proceedWithVerification(message, userId, currency, prefix, interaction = null) {
      ...
      // same nonce dance as btc but with extra mainnet pretentiousness 😭
      ...
      try {
        ...
        // if this throws, 90% of the time it's "open your dms" and 10% cosmic rays 😭
    ...
    // friendly reminder that `eth gwei` is not a subcommand (i tried)
    return message.reply({
      embeds: [
        new EmbedBuilder()
          .setColor('#838996')
