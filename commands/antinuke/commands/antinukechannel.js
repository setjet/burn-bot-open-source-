...
module.exports = {
  ...
  execute: async (message, args, { prefix }) => {
    ...
    if (status === 'off') {
      config.modules.channel.enabled = false;
      saveAntinukeConfig(message.guild.id, config);
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setColor('#FF0000')
            .setDescription('<:check:1362850043333316659> <:arrows:1363099226375979058> **channel** module disabled.')
        ]
      });
    }

    config.modules.channel.enabled = true;
    ...
    if (flags.threshold !== undefined) {
      ...
      config.modules.channel.threshold = flags.threshold;
    }
    ...
    if (flags.punishment) {
      ...
      config.modules.channel.punishment = flags.punishment;
    }

    saveAntinukeConfig(message.guild.id, config);
    
    const threshold = config.modules.channel.threshold;
    const punishment = config.modules.channel.punishment;
    ...
    return message.reply({
      embeds: [
        new EmbedBuilder()
          .setColor('#57F287')
          .setDescription([
            `<:check:1362850043333316659> <:arrows:1363099226375979058> **channel** module enabled.`,
